...
local function AddMasterWrits(bag)
	for slot = 1, 480 do
		local itemType = GetItemType(bag, slot)
		local link = GetItemLink(bag, slot)
		-- if itemType~=0 then d("link " .. link .. " itemType " .. itemType) end
		if itemType==ITEMTYPE_MASTER_WRIT then
			...
			if station then
				if station == CRAFTING_TYPE_ENCHANTING then
					...
					local flavour = GetItemLinkFlavorText(link)
...
function WritCreater.InventorySlot_ShowContextMenu(rowControl,debugslot)
    ...
    zo_callLater(function ()
        AddCustomMenuItem("Craft All Sealed Writs " .. #trackedSealedWrits, function ()
			AddMasterWrits(BAG_BANK)
			AddMasterWrits(BAG_SUBSCRIBER_BANK)
